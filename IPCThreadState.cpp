...
void IPCThreadState::flushCommands()
{
    ...
    if (mInTalkWithDriver > 0) {
        IF_LOG_NESTEDCOMMANDS() {
            ALOGE("%s: Already talking with driver! mInTalkwithDriver:%d", __func__, mInTalkWithDriver);
        }
...
status_t IPCThreadState::talkWithDriver(Parcel *in, Parcel *out, bool doReceive)
{
    ...
    if (mOut1.dataSize() > 0) {
        IF_LOG_NESTEDCOMMANDS() {
            ALOGE("%s: Merging pending message from mOut1 (data:%p dataSize:%d) to out (data:%p dataSize:%d)",
    ...
    if (mInTalkWithDriver > 0) {
        IF_LOG_NESTEDCOMMANDS() {
            ALOGE("%s: Merging pending...... mOut1 (data:%p dataSize:%d) to out (data:%p dataSize:%d)",
                __func__, mOut1.data(), mOut1.dataSize(), out->data(), out->dataSize());
        }
    }
    mInTalkWithDriver++;
    IF_LOG_NESTEDCOMMANDS() {
        ALOGE("%s: Entering mInTalkWithDriver:%d in:%p out:%p", __func__, mInTalkWithDriver, in , out);
    }
    ...
    if ((bwr.write_size == 0) && (bwr.read_size == 0)) {
        IF_LOG_NESTEDCOMMANDS() {
            ALOGE("%s: Exit mInTalkWithDriver:%d in:%p out:%p err:NO_ERROR", __func__, mInTalkWithDriver, in , out);
            mInTalkWithDriver--;
            return NO_ERROR;
        }
    }
    ...
    do {
        ...
        IF_LOG_NESTEDCOMMANDS() {
            ALOGE("%s: Going to read/write (%p), write size = %d in:%p out:%p write_consumed:%llu",
        ...
        IF_LOG_NESTEDCOMMANDS() {
            ALOGE("%s: Done read/write (%p), write size = %d in:%p out:%p err:%d write_consumed:%llu",
    ...
    IF_LOG_NESTEDCOMMANDS() {
        ALOGE("%s: Exit mInTalkWithDriver:%d in:%p out:%p err:%d", __func__, mInTalkWithDriver, in, out, err);
        mInTalkWithDriver--;
    }
                                ...
                                size_t /*objectsSize*/, void* /*cookie*/)
{
    ...
    ALOGE("%s: ======== mInTalkWithDriver:%d", __func__, state->mInTalkWithDriver);
